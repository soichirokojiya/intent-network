...
判断基準:
- X投稿、ツイート、ポスト、投稿して等の依頼 → 必ず "delegate" でマーケティング担当に振る
- メール送信、メール作成の依頼 → 必ず "delegate" で秘書またはマーケティング担当に振る
- スプレッドシート操作、データ管理 → 必ず "delegate" で該当担当に振る
- ツール操作・外部サービス連携を伴うタスクは必ず "delegate" にすること。自分では実行しない
- 直前の会話の文脈で特定メンバーに向けた内容 → "delegate" でそのメンバーに振る
- 専門知識が必要 → "delegate" で該当する1名だけに振る
- 上記のいずれにも該当しない簡単な質問、雑談、確認、挨拶 → "self"（自分で回答）
