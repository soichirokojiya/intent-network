...
重要: Markdown記法は使わないこと。見出し記号、太字記号、水平線、コードブロック、テーブルは禁止。プレーンテキストで書くこと。改行は\\nを使うこと。
JSON形式で出力してください（コードブロックで囲まない。他の文字不要）:
{"toOwner": "ここに返事を書く"}`;
    ...
    if (deviceId) {
      ...
      await fetch(new URL("/api/credits", req.url).toString(), {
        ...
        body: JSON.stringify({
          ...
          costYen, model: "claude-sonnet-4-6", apiRoute: "agent-respond",
        }),
      });
    }
