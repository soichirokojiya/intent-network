...
export async function POST(req: NextRequest) {
  try {
    const { ownerMessage, orchestratorName, orchestratorPersonality, orchestratorTone, agents, conversationHistory, ownerBusinessInfo } = await req.json();
...
ルール:
- アクティブな全メンバーにそれぞれの専門に合ったタスクを振る。誰も外さない
- 各メンバーの専門性を活かした固有の視点でタスクを与える
