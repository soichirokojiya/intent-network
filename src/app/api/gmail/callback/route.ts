...
export async function GET(req: NextRequest) {
  ...
  if (error) {
    return NextResponse.redirect(new URL("/integrations?gmail=error", req.url));
  }
  ...
  try {
    ...
    const tokenRes = await fetch("https://oauth2.googleapis.com/token", {
      ...
      body: new URLSearchParams({
        ...
        redirect_uri: "https://musu.world/api/gmail/callback",
    ...
    return NextResponse.redirect(new URL("/integrations?gmail=connected", req.url));
  } catch (err) {
