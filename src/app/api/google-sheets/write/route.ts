...
export async function POST(req: NextRequest) {
  const { deviceId, spreadsheetId, range, values } = await req.json();
  if (!deviceId || !spreadsheetId || !range || !values) {
    return NextResponse.json({ error: "deviceId, spreadsheetId, range, and values required" }, { status: 400 });
  }
  ...
  if (!apiRes.ok) {
    ...
    return NextResponse.json({ error: "Failed to write to spreadsheet", details: errData }, { status: apiRes.status });
  }

  const data = await apiRes.json();
  return NextResponse.json({ updatedRange: data.updatedRange, updatedRows: data.updatedRows, updatedColumns: data.updatedColumns, updatedCells: data.updatedCells });
}
